...
* `par_byyear` # of time participated in the swim meet prior to 2019 - regardless of the events participated, so max per person is 1 per year  
* `gender`   
* `fastfish`     
* `axis`      
* `baseage`  
...
After taking a few looks of my data, I firstly use one variable at a time, and then compare it with the full model to see how the coefficient changed comparing unconditional and conditional logistic regression.
...
+ 1650 Freestyle: I use this to evaluate the speed of long-distance swimming  
...
From the figures, it seems that there are not very much between individual variations among non-fast fishes (so non-fast fishes are a more homogenous group), but there are lots of between and within individual variations among fast fishes, which can be contributed by the baseline age. 
...
**Ideally:**
Assuming that everyone's speed changes in the same way over time, but that some people's trajectories are going to be shifted higher or lower depending on their gender, baseline age (at their first entry to the race), swimmer type, and their "fish type".   
...
**In reality:**
I soon found out that by adding year as a random and fixed effect, the degree of freedom (df) lost quickly. The more years and the more people we have in the mixed model, the less df we have, and the less likely the model converges.
...
Year (aging) is the variable that I am interested in. We see that the estimate actually looks similar comparing three distinct events. Gender is an influential effect modifier across all three events. I would not go into too much depth because of the degree of freedom we have to fit the mixed model.

Something I also want to highlight is the ICC: the ratio of the between-individual variance to the total variance.  Here ICC is all around 0.95, so we have more between individual variations than within individual variations! We probability do not need to fit the mixed model, IF WE CHANGE OUR STUDY QUESTION, and only interested in population level change.

To find more about marginal model, please refer to the visualization page.

Obviously, I ran out of my capability to tell the whole story, but I think there are more we can learn from this dataset. If you are interested in doing more, please find my contact information at the bottom of this page. 

***  
#### Limitations!
## Limitations!!

I have to shout out to the world on this…despite it is at the very end of the page…

* Selection bias: Basically, people did not show up in the following years simply because…that they moved, which could be non-informative. Other informative censoring, such as injury from swimming, can also be true. Showing up in a meet also indicates that they are healthy enough to participate in a meet. This could be a “healthy swimmer effect”. 
There is no way we can know whether the person is loss to follow or they do not want to participate in the meet based on our data.  This is a huge caveat to our analysis. (I also admit that my analysis is SUPER preliminary due to the nature of data, and maybe VERY biased due to omitted variables, model misspecification, etc.)

* A reminder that my data only focus on the individual events, which mean that participation in the relay is not counted. It could totally be true to say that some people are more social animals and more likely to attend team events vs individual events.

***

### Finally...  
Finally, because this is a data sciences class project, my work heavily lean towards the quantitive side. However, my belief is that data sciences are more than numbers. There are stories, qualitatively, giving you insights about the social connection between swimmers, and the benefits that swimmers can gain by joining a group. 

So I encourage you to network with your fellow lane mates. Learn about their motivations. I wish data could be an incentive for anyone to open conversations with their teammates and/or your coaches. And sometimes conversations can lead to meaningful changes.

In the long run, I wish to continuously working on this project in my spare time (too bad it cannot be a thesis…). If you want to collaborate, please contact :)
